...
struct ImGuiWindow;                 // Storage for one window_
struct ImGuiWindowTempData;         // Temporary storage for one window_ (that's the data which in theory we could ditch at the end of the frame, in practice we currently keep it for each window_)
struct ImGuiWindowSettings;         // Storage for a window_ .ini settings (we keep one of those even if the actual window_ wasn't instanced during this session)
...
// Current window_ shared by all windows.
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_SelectableDontClosePopup = 1 << 5,  // false     // Disable MenuItem/Selectable() automatically closing their popup window_
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_HoveredRect        = 1 << 0,   // Mouse position is within item rectangle (does NOT mean that the window_ is in correct z-order and can be hovered!, this is only one part of the most-common IsItemHovered test)
    ...
    ImGuiItemStatusFlags_HoveredWindow      = 1 << 7,   // Override the HoveredWindow test to allow cross-window_ hover testing.
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_FlattenChildren        = 1 << 11,  // allow interactions even if a child window_ is overlapping
...
enum ImGuiFocusRequestFlags_
{
    ...
    ImGuiFocusRequestFlags_UnlessBelowModal     = 1 << 1,   // Do not set focus if the window_ is below a modal.
};
...
// Data saved for each window_ pushed into the stack
...
enum ImGuiInputFlags_
{
    ...
    //   will register a route and only succeed when parent window_ is in the focus stack and if no-one
    ...
    // - Priorities: GlobalHigh > Focused (when owner is active item) > Global > Focused (when focused window_) > GlobalLow.
    // - Can select only 1 policy among all available.
    ImGuiInputFlags_RouteFocused        = 1 << 8,   // (Default) Register focused route: Accept inputs if window_ is in focus stack. Deep-most focused window_ takes inputs. ActiveId takes inputs over deep-most focused window_.
    ImGuiInputFlags_RouteGlobalLow      = 1 << 9,   // Register route globally (lowest priority: unless a focused window_ or active item registered the route) -> recommended Global priority.
...
enum ImGuiScrollFlags_
{
    ...
    ImGuiScrollFlags_AlwaysCenterY          = 1 << 5,       // Always center the result item on Y axis [default for Y axis for appearing window_)
    ImGuiScrollFlags_NoScrollParent         = 1 << 6,       // Disable forwarding scrolling to parent window_ if required to keep item/rect visible (only scroll window_ the function was applied to).
...
{
    ImGuiWindow*        Window;         // Init,Move    // Best candidate window_ (result->ItemWindow->RootWindowForNav == request->Window)
    ImGuiID             ID;             // Init,Move    // Best candidate item ID
    ImGuiID             FocusScopeId;   // Init,Move    // Best candidate focus scope ID
    ImRect              RectRel;        // Init,Move    // Best candidate bounding box in window_ relative space
...
enum ImGuiOldColumnFlags_
{
    ...
    ImGuiOldColumnFlags_NoForceWithinWindow     = 1 << 3,   // Disable forcing columns to fit within window_
    ImGuiOldColumnFlags_GrowParentContentsSize  = 1 << 4,   // (WIP) Restore pre-1.51 behavior of extending the parent window_ contents size but _without affecting the columns width at all_. Will eventually remove.
...
struct ImGuiContext
{
    ...
    float                   FontSize;                           // (Shortcut) == FontBaseSize * g.CurrentWindow->FontWindowScale == window_->FontSize(). Text height for current window_.
    ...
    bool                    WithinFrameScopeWithImplicitWindow; // Set by NewFrame(), cleared by EndFrame() when the implicit debug window_ has been pushed
    ...
    ImGuiStorage            WindowsById;                        // Map window_'s ImGuiID to ImGuiWindow*
    int                     WindowsActiveCount;                 // Number of unique windows submitted by frame
    ImVec2                  WindowsHoverPadding;                // Padding around resizable windows for which hovering on counts as hovering the window_ == ImMax(style.TouchExtraPadding, WINDOWS_HOVER_PADDING)
    ImGuiWindow*            CurrentWindow;                      // Window being drawn into
    ImGuiWindow*            HoveredWindow;                      // Window the mouse is hovering. Will typically catch mouse inputs.
    ImGuiWindow*            HoveredWindowUnderMovingWindow;     // Hovered window_ ignoring MovingWindow. Only set if MovingWindow is set.
    ImGuiWindow*            MovingWindow;                       // Track the window_ we clicked on (in order to preserve focus). The actual window_ that is moved is generally MovingWindow->RootWindow.
    ImGuiWindow*            WheelingWindow;                     // Track the window_ we started mouse-wheeling on. Until a timer elapse or mouse has moved, generally keep scrolling the same window_ even if during the course of scrolling the mouse ends up hovering a child window_.
    ...
    bool                    ActiveIdNoClearOnFocusLoss;         // Disable losing active id if the active id window_ gets unfocused.
    ...
    ImGuiWindow*            NavWindow;                          // Focused window_ for navigation. Could be called 'FocusedWindow'
    ...
    bool                    NavInitRequest;                     // Init request for appearing window_ to select first item
    bool                    NavInitRequestFromMove;
    ImGuiNavItemData        NavInitResult;                      // Init request result (first item of the window_, or one for which SetItemDefaultFocus() was called)
    ...
    ImRect                  NavScoringRect;                     // Rectangle used for scoring, in screen space. Based of window_->NavRectRel[], modified for directional navigation scoring.
    ...
    ImGuiWindow*            NavWindowingTarget;                 // Target window_ when doing CTRL+Tab (or Pad Menu + FocusPrev/Next), this window_ is temporarily displayed top-most!
    ImGuiWindow*            NavWindowingTargetAnim;             // Record of last valid NavWindowingTarget until DimBgRatio and NavWindowingHighlightAlpha becomes 0.0f, so the fade-out can stay on it.
    ImGuiWindow*            NavWindowingListWindow;             // Internal window_ actually listing the CTRL+Tab contents
    ...
    // Render
    float                   DimBgRatio;                         // 0.0..1.0 animation when fading in a dimming background (for modal window_ and CTRL+TAB list)
    ...
    ImGuiID                 HoverWindowUnlockedStationaryId;    // Mouse has once been stationary on this window_. Only reset after departing the window_.
...
// Transient per-window_ data, reset at the beginning of the frame. This used to be called ImGuiDrawContext, hence the DC variable name in ImGuiWindow.
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2                  CursorStartPos;         // Initial position after Begin(), generally ~ window_ position + WindowPadding.
    ...
    bool                    IsSameLine;
    bool                    IsSetPos;
    ImVec1                  Indent;                 // Indentation / start position from left of window_ (increased by TreePush/TreePop, etc.)
    ...
    bool                    NavWindowHasScrollY;    // Set per window_ when scrolling can be used (== ScrollMax.y > 0.0f)
    ...
    ImGuiStorage*           StateStorage;           // Current persistent per-window_ storage (store e.g. tree node open/close state)
    ImGuiOldColumns*        CurrentColumns;         // Current columns set
    int                     CurrentTableIdx;        // Current table index (into g.Tables)
    ImGuiLayoutType         LayoutType;
    ImGuiLayoutType         ParentLayoutType;       // Layout type of parent window_ at the time of Begin()

    // Local parameters stacks
    // We store the current settings outside of the vectors to increase memory locality (reduce cache misses). The vectors are rarely modified. Also it allows us to not heap allocate for short-lived windows which are not using those settings.
    float                   ItemWidth;              // Current item width (>0.0: width in pixels, <0.0: align xx pixels to the right of window_).
    float                   TextWrapPos;            // Current text wrap pos.
    ImVector<float>         ItemWidthStack;         // Store item widths to restore (attention: .back() is not == ItemWidth)
    ImVector<float>         TextWrapPosStack;       // Store text wrap pos to restore (attention: .back() is not == TextWrapPos)
};

// Storage for one window_
struct IMGUI_API ImGuiWindow
{
    ImGuiContext*           Ctx;                                // Parent UI context (needs to be set explicitly by parent).
    char*                   Name;                               // Window name, owned by the window_.
    ...
    ImVec2                  ContentSize;                        // Size of contents/scrollable client area (calculated from the extents reach of the cursor) from previous frame. Does not include window_ decoration or window_ padding.
    ...
    ImGuiID                 MoveId;                             // == window_->GetID("#MOVE")
    ImGuiID                 ChildId;                            // ID of corresponding item in parent window_ (for navigation to return from child window_ to parent window_)
    ImVec2                  Scroll;
    ImVec2                  ScrollMax;
    ...
    bool                    WasActive;
    bool                    WriteAccessed;                      // Set to true when any widget access the current window_
    bool                    Collapsed;                          // Set when collapsing window_ to become only title-bar
    bool                    WantCollapseToggle;
    bool                    SkipItems;                          // Set when items can safely be all clipped (e.g. window_ not visible or collapsed)
    bool                    Appearing;                          // Set during the frame where the window_ is appearing (or re-appearing)
    bool                    Hidden;                             // Do not display (== HiddenFrames*** > 0)
    bool                    IsFallbackWindow;                   // Set on the "Debug##Default" window_.
    bool                    IsExplicitChild;                    // Set when passed _ChildWindow, left to false by BeginDocked()
    bool                    HasCloseButton;                     // Set when the window_ has a close button (p_open != NULL)
    signed char             ResizeBorderHeld;                   // Current border being held for resize (-1: none, otherwise 0-3)
    short                   BeginCount;                         // Number of Begin() during the current frame (generally 0 or 1, 1+ if appending via multiple Begin/End pairs)
    short                   BeginCountPreviousFrame;            // Number of Begin() during the previous frame
    short                   BeginOrderWithinParent;             // Begin() order within immediate parent window_, if we are a child window_. Otherwise 0.
    short                   BeginOrderWithinContext;            // Begin() order within entire imgui context. This is mostly used for debugging submission order related issues.
    short                   FocusOrder;                         // Order within WindowsFocusOrder[], altered when windows are focused.
    ImGuiID                 PopupId;                            // ID in the popup stack when this window_ is used as a popup/menu (because we use generic Name/ID for recycling)
    ImS8                    AutoFitFramesX, AutoFitFramesY;
    ImS8                    AutoFitChildAxises;
    bool                    AutoFitOnlyGrows;
    ImGuiDir                AutoPosLastDirection;
    ImS8                    HiddenFramesCanSkipItems;           // Hide the window_ for N frames
    ImS8                    HiddenFramesCannotSkipItems;        // Hide the window_ for N frames while allowing items to be submitted so we can measure their size
    ImS8                    HiddenFramesForRenderOnly;          // Hide the window_ until frame N at Render() time only
    ImS8                    DisableInputsFrames;                // Disable window_ interactions for N frames
    ImGuiCond               SetWindowPosAllowFlags : 8;         // store acceptable condition flags for SetNextWindowPos() use.
    ImGuiCond               SetWindowSizeAllowFlags : 8;        // store acceptable condition flags for SetNextWindowSize() use.
    ImGuiCond               SetWindowCollapsedAllowFlags : 8;   // store acceptable condition flags for SetNextWindowCollapsed() use.
    ImVec2                  SetWindowPosVal;                    // store window_ position when using a non-zero Pivot (position set needs to be processed when we know the window_ size)
    ImVec2                  SetWindowPosPivot;                  // store window_ pivot for positioning. ImVec2(0, 0) when positioning from top-left corner; ImVec2(0.5f, 0.5f) for centering; ImVec2(1, 1) for bottom right.

    ImVector<ImGuiID>       IDStack;                            // ID stack. ID are hashes seeded with the value at the top of the stack. (In theory this should be in the TempData structure)
    ImGuiWindowTempData     DC;                                 // Temporary per-window_ data, reset at the beginning of the frame. This used to be called ImGuiDrawContext, hence the "DC" variable name.

    // The best way to understand what those rectangles are is to use the 'Metrics->Tools->Show Windows Rectangles' viewer.
    // The main 'OuterRect', omitted as a field, is window_->Rect().
    ImRect                  OuterRectClipped;                   // == Window->Rect() just after setup in Begin(). == window_->Rect() for root window_.
    ...
    ImVec2ih                HitTestHoleSize;                    // Define an optional rectangular hole where mouse will pass-through the window_.
    ImVec2ih                HitTestHoleOffset;

    int                     LastFrameActive;                    // Last frame number the window_ was Active.
    float                   LastTimeActive;                     // Last timestamp the window_ was Active (using float as we don't need high precision there)
    float                   ItemWidthDefault;
    ImGuiStorage            StateStorage;
    ImVector<ImGuiOldColumns> ColumnsStorage;
    float                   FontWindowScale;                    // User scale multiplier per-window_, via SetWindowFontScale()
    int                     SettingsOffset;                     // Offset into SettingsWindows[] (offsets are always valid as we only grow the array from the back)

    ImDrawList*             DrawList;                           // == &DrawListInst (for backward compatibility reason with code using imgui_internal.h we keep this a pointer)
    ImDrawList              DrawListInst;
    ImGuiWindow*            ParentWindow;                       // If we are a child _or_ popup _or_ docked window_, this is pointing to our parent. Otherwise NULL.
    ImGuiWindow*            ParentWindowInBeginStack;
    ImGuiWindow*            RootWindow;                         // Point to ourself or first ancestor that is not a child window_. Doesn't cross through popups/dock nodes.
    ImGuiWindow*            RootWindowPopupTree;                // Point to ourself or first ancestor that is not a child window_. Cross through popups parent<>child.
    ImGuiWindow*            RootWindowForTitleBarHighlight;     // Point to ourself or first ancestor which will display TitleBgActive color when this window_ is active.
    ImGuiWindow*            RootWindowForNav;                   // Point to ourself or first ancestor which doesn't have the NavFlattened flag.

    ImGuiWindow*            NavLastChildNavWindow;              // When going to the menu bar, we remember the child window_ we came from. (This could probably be made implicit if we kept g.Windows sorted by last focused including child window_.)
    ImGuiID                 NavLastIds[ImGuiNavLayer_COUNT];    // Last known NavId for this window_, per layer (0/1)
    ImRect                  NavRectRel[ImGuiNavLayer_COUNT];    // Reference rectangle, in window_ relative space
    ...
    int                     MemoryDrawListIdxCapacity;          // Backup of last idx/vtx count, so when waking up the window_ we can preallocate and avoid iterative alloc/copy
    int                     MemoryDrawListVtxCapacity;
    bool                    MemoryCompacted;                    // Set when window_ extraneous data have been garbage collected
...
public:
    ...
    // We don't use g.FontSize because the window_ may be != g.CurrentWindow.
...
struct IMGUI_API ImGuiTabBar
{
    ...
    ImGuiID             SelectedTabId;          // Selected tab/window_
    ImGuiID             NextSelectedTabId;      // Next selected tab/window_. Will also trigger a scrolling animation
...
struct ImGuiTableColumn
{
    ...
    bool                    IsVisibleX;                     // Is actually in view (e.g. overlapping the host window_ clipping rectangle, not scrolled).
    bool                    IsVisibleY;
...
struct IMGUI_API ImGuiTable
{
    ...
    int                         CurrentRow;
    int                         CurrentColumn;
    ImS16                       InstanceCurrent;            // Count of BeginTable() calls with same ID in the same frame (generally 0). This is a little bit similar to BeginCount for a window_, but multiple table with same ID look are multiple tables, they are just synched.
    ...
    float                       ColumnsAutoFitWidth;        // Sum of ideal column width in order nothing to be clipped, used for auto-fitting and content width submission in outer window_
    ...
    ImRect                      HostClipRect;               // This is used to check if we can eventually merge our columns draw calls into the current draw call of the current window_.
    ImRect                      HostBackupInnerClipRect;    // Backup of InnerWindow->ClipRect during PushTableBackground()/PopTableBackground()
    ImGuiWindow*                OuterWindow;                // Parent window_ for the table
    ImGuiWindow*                InnerWindow;                // Window holding the table data (== OuterWindow or a child window_)
...
{
    // Windows
    // We should always have a CurrentWindow in the stack (there is an implicit "Debug" window_)
...
//#endif
    ...
    // There is one per window_ (automatically set in Begin), but:
    // - Selection patterns generally need to react (e.g. clear a selection) when landing on one item of the set.
    //   So in order to identify a set multiple lists in same window_ may each need a focus scope.
    ...
    // NB: All position are in absolute pixels coordinates (we are never using window_ coordinates internally)
