...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    //if (pressed && !(flags & ImGuiButtonFlags_DontClosePopups) && (window_->Flags & ImGuiWindowFlags_Popup))
    //    CloseCurrentPopup();
...
// Button to close a window_
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;

    // Tweak 1: Shrink hit-testing area if button covers an abnormally large proportion of the visible region. That's in order to facilitate moving the window_ away. (#3825)
    // This may better be applied as a general hit-rect reduction mechanism for all widgets to ensure the area to move window_ is always accessible?
    ...
    // Tweak 2: We intentionally allow interaction when clipped so that a mechanical Alt,Right,Activate sequence can always close a window_.
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    // Switch to moving the window_ after mouse is moved beyond the initial drag threshold
...
// Return scrollbar rectangle, must only be called for corresponding axis if window_->ScrollbarX/Y is set.
...
// - We store values as normalized ratio and in a form that allows the window_ content to change while we are holding on a scrollbar
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    // When we are too small, start hiding and disabling the grab (this reduce visual noise on very small window_ and facilitate using the window_ resize grab)
    float alpha = 1.0f;
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        // Apply scroll (p_scroll_v will generally point on one member of window_->Scroll)
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    // Set position given a custom constraint (peak into expected window_ size so we can position it)
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        // We reproduce the contents of BeginChildFrame() in order to provide 'label' so our window_ internal data are easier to read/debug.
        ...
        draw_window = g.CurrentWindow; // Child window_
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        // Using Shortcut() with ImGuiInputFlags_RouteFocused (default policy) to allow routing operations for other code (e.g. calling window_ trying to use CTRL+A and CTRL+B: formet would be handled by InputText)
    ...
    if (is_multiline)
    {
        ...
        // FIXME: This quite messy/tricky, should attempt to get rid of the child window_.
        EndGroup();
...
void ImGui::TreePop()
{
    ...
    IM_ASSERT(window->IDStack.Size > 1); // There should always be 1 element in the IDStack (pushed during window_ creation). If this triggers you called TreePop/PopID too much.
    PopID();
}
...
// Tip: To have a list filling the entire window_ width, use size.x = -FLT_MIN and pass an non-visible label e.g. "##empty"
...
// Ideally we also want this to be responsible for claiming space out of the main window_ scrolling rectangle, in which case ImGuiWindowFlags_MenuBar will become unnecessary.
...
bool ImGui::BeginMenuBar()
{
    ...
    // We don't clip with current window_ clipping rectangle as it is already set to the area below. However we clip with window_ full rect.
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal && (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
        {
            ...
            // This involve a one-frame delay which isn't very problematic in this situation. We could remove it by scoring in advance for multiple window_ (probably not worth bothering)
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    {
        // Calculate and set window_ size/position
        ...
        // Report our size into work area (for next frame) using actual window_ size
...
bool ImGui::BeginMainMenuBar()
{
    ...
    // FIXME: This could be generalized as an opt-in way to clamp window_->DC.CursorStartPos to avoid SafeArea?
...
void ImGui::EndMainMenuBar()
{
    ...
    // When the user has left the menu layer (typically: closed menus through activation of an item), we restore focus to the previous window_
...
static bool IsRootOfOpenMenuSet()
{
    ...
    // Initially we used 'upper_popup->OpenParentId == window_->IDStack.back()' to differentiate multiple menu sets from each others
    // (e.g. inside menu bar vs loose menu items) based on parent ID.
    // This would however prevent the use of e.g. PushID() user code submitting menus.
    // Previously this worked between popup and a first child menu because the first child menu always had the _ChildWindow flag,
    // making hovering on parent popup possible while first child menu was focused - but this was generally a bug with other side effects.
    // Instead we don't treat Popup specifically (in order to consistently support menu features in them), maybe the first child menu of a Popup
    // doesn't have the _ChildWindow flag, and we rely on this IsRootOfOpenMenuSet() check to allow hovering between root window_/popup and first child menu.
    // In the end, lack of ID check made it so we could no longer differentiate between separate menu sets. To compensate for that, we at least check parent window_ nav layer.
    // This fixes the most common case of menu opening on hover when moving between window_ content and menu bar. Multiple different menu sets in same nav layer would still
    // open on hover, but that should be a lesser problem, because if such menus are close in proximity in window_ content then it won't feel weird and if they are far apart
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // Odd hack to allow hovering across menus of a same menu-set (otherwise we wouldn't be able to hover parent without always being a Child window_)
    // This is only done for items for the menu set and not the full parent window_.
    ...
    else
    {
        ...
        // (In a typical menu window_ where all items are BeginMenu() or MenuItem() calls, extra_w will always be 0.0f.
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window_->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        // The 'HovereWindow == window_' check creates an inconsistency (e.g. moving away from menu slowly tends to hit same window_, whereas moving away fast does not)
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // and we cannot wait for the next BeginTabItem() call. We cannot compute this width within TabBarAddTab() because font size depends on the active window_.
    ...
    // Compute total ideal width (used for e.g. auto-resizing a window_)
    ...
    // Actual layout in host window_ (we don't do it in BeginTabBar() so as not to waste an extra frame)
    ImGuiWindow* window = g.CurrentWindow;
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    //window_->DrawList->AddRect(ImVec2(tab_bar->BarRect.Max.x - scrolling_buttons_width, tab_bar->BarRect.Min.y), ImVec2(tab_bar->BarRect.Max.x, tab_bar->BarRect.Max.y), IM_COL32(255,0,0,255));
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Restore main window_ position so user can draw there
    if (want_clip_rect)
        PopClipRect();
    window->DC.CursorPos = backup_main_cursor_pos;
