...
//   X
//   - outer_size.x <= 0.0f  ->  Right-align from window_/work-rect right-most edge. With -FLT_MIN or 0.0f will align exactly on right-most edge.
//   - outer_size.x  > 0.0f  ->  Set Fixed width.
//   Y with ScrollX/ScrollY disabled: we output table directly in current window_
//   - outer_size.y  < 0.0f  ->  Bottom-align (but will auto extend, unless _NoHostExtendY is set). Not meaningful if parent window_ can vertically scroll.
//   - outer_size.y  = 0.0f  ->  No minimum height (but will auto extend, unless _NoHostExtendY is set)
//   - outer_size.y  > 0.0f  ->  Set Minimum height (but will auto extend, unless _NoHostExtendY is set)
//   Y with ScrollX/ScrollY enabled: using a child window_ for scrolling
//   - outer_size.y  < 0.0f  ->  Bottom-align. Not meaningful if parent window_ can vertically scroll.
//   - outer_size.y  = 0.0f  ->  Bottom-align, consistent with BeginChild(). Not recommended unless table is last item in parent window_.
...
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags, ImGuiWindow* outer_window)
{
    ...
    // Inherit _NoSavedSettings from top-level window_ (child windows always have _NoSavedSettings set)
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // When not using a child window_, WorkRect.Max will grow as we append contents.
    if (use_child_window)
    {
        // Ensure no vertical scrollbar appears if we only want horizontal one, to make flag consistent
        // (we have no other way to disable vertical scrollbar of a window_ while keeping the horizontal one showing)
        ...
        // based on the right side of the child window_ work rect, which would require knowing ahead if we are going to
        ...
        // Create scrolling region (without border and zero window_ padding)
    ...
    // Backup a copy of host window_ members we will modify
    ...
    if (inner_window != outer_window) // So EndChild() within the inner window_ can restore the table properly.
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    // [Part 2] Disable child window_ clipping while fitting columns. This is not strictly necessary but makes it possible
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Possible solution to preserve last known content width for clipped column. Test 'table_reported_size' fails when enabling Y clipping and window_ is resized small.
        ...
        // Don't decrement auto-fit counters until container window_ got a chance to submit its items
    ...
    // Setup window_'s WorkRect.Max.y for GetContentRegionAvail(). Other values will be updated in each TableBeginCell() call.
    ...
    // [Part 13] Setup inner window_ decoration size (for scrolling / nav tracking to properly take account of frozen rows/columns)
...
void    ImGui::EndTable()
{
    ...
    // Restore window_ data that we modified
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    if (column->Flags & ImGuiTableColumnFlags_IndentEnable)
        start_x += table->RowIndentOffsetX; // ~~ += window_.DC.Indent.x - table->HostIndentX, except we locked it for the row.
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                // - On a same-window_ table (not scrolling = single group), all fitting columns ClipRect -> will extend and match host ClipRect -> will merge
                // - Columns can use padding and have left-most ClipRect.Min.x and right-most ClipRect.Max.x != from host ClipRect -> will extend and match host ClipRect -> will merge
                // FIXME-TABLE FIXME-WORKRECT: We are wasting a merge opportunity on tables without scrolling if column doesn't fit
                // within host clip rect, solely because of the half-padding difference between window_->WorkRect and window_->InnerClipRect.
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // Draw in outer window_ so right-most column won't be clipped
    ...
    if (table->Flags & ImGuiTableFlags_BordersOuter)
    {
        ...
        // of it in inner window_, and the part that's over scrollbars in the outer window_..)
...
void ImGui::TableHeader(const char* label)
{
    ...
    //window_->DrawList->AddCircleFilled(ImVec2(ellipsis_max, label_pos.y), 40, IM_COL32_WHITE);
...
// Output context menu into current window_ (generally a popup)
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    // window_ creates a feedback loop because we store normalized positions. So while dragging we enforce absolute positioning.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
{
    // We have few columns per window_ so for now we don't need bother much with turning this into a faster lookup.
